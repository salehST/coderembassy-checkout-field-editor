...
class FieldsController extends BaseController {
	...
	public function create( WP_REST_Request $request ) {
		...
		$type             = $payload['type'] ?? '';
		$has_pricing_rules = ! empty( $payload['pricing_rules'] );
		if (
			( 'custom_price' === $type || $has_pricing_rules )
			&& ! \CoderEmbassy\CheckoutFieldEditor\Modules\Licensing\FeatureGate::can(
				\CoderEmbassy\CheckoutFieldEditor\Modules\Licensing\FeatureGate::PRICING_ENGINE
			)
		) {
			return $this->error( 'Pricing rules and the custom_price field type are Pro features.', 403 );
		}

		// Free tier: max 3 custom fields.
		if ( ! \CoderEmbassy\CheckoutFieldEditor\Modules\Licensing\FeatureGate::isPro() ) {
			$existing_count = count( $this->repository->findAll( array() ) );
			if ( $existing_count >= \CoderEmbassy\CheckoutFieldEditor\Modules\Licensing\FeatureGate::FREE_FIELDS_MAX ) {
				return $this->error(
					sprintf(
						'Free plan allows up to %d custom fields. Upgrade to Pro for unlimited fields.',
						\CoderEmbassy\CheckoutFieldEditor\Modules\Licensing\FeatureGate::FREE_FIELDS_MAX
					),
					403
				);
			}
		...
		if ( 'repeater' === (string) ( $payload['type'] ?? '' ) && ! \CoderEmbassy\CheckoutFieldEditor\Modules\Licensing\FeatureGate::isPro() ) {
			return $this->error( 'Repeater fields are a Pro feature. Please activate your license.', 403 );
		}

		// Free tier: max 1 condition rule per field (single rule, no groups).
		if ( ! \CoderEmbassy\CheckoutFieldEditor\Modules\Licensing\FeatureGate::isPro() ) {
			$conditions = $payload['conditions'] ?? array();
			$groups     = is_array( $conditions ) ? ( $conditions['groups'] ?? array() ) : array();
			if ( count( $groups ) > 1 ) {
				return $this->error( 'Free plan supports only 1 condition rule per field. Upgrade to Pro for condition groups.', 403 );
			}
			if ( count( $groups ) === 1 && count( $groups[0]['rules'] ?? array() ) > \CoderEmbassy\CheckoutFieldEditor\Modules\Licensing\FeatureGate::FREE_CONDITIONS_MAX ) {
				return $this->error( 'Free plan supports only 1 condition rule per field. Upgrade to Pro for multiple rules.', 403 );
			}
		}

		$field = CA_Field::fromArray( $payload );
	...
	public function update( WP_REST_Request $request ) {
		...
		$has_pricing_rules = array_key_exists( 'pricing_rules', $payload ) ? ! empty( $payload['pricing_rules'] ) : ! empty( $field->pricing_rules );
		if (
			( 'custom_price' === $type || $has_pricing_rules )
			&& ! \CoderEmbassy\CheckoutFieldEditor\Modules\Licensing\FeatureGate::can(
				\CoderEmbassy\CheckoutFieldEditor\Modules\Licensing\FeatureGate::PRICING_ENGINE
		...
		if ( 'repeater' === (string) $type && ! \CoderEmbassy\CheckoutFieldEditor\Modules\Licensing\FeatureGate::isPro() ) {
			return $this->error( 'Repeater fields are a Pro feature. Please activate your license.', 403 );
		}

		// Free tier: max 1 condition rule per field.
		if ( ! \CoderEmbassy\CheckoutFieldEditor\Modules\Licensing\FeatureGate::isPro() ) {
			$conditions = $payload['conditions'] ?? ( is_array( $field->conditions ) ? $field->conditions : array() );
			$groups     = is_array( $conditions ) ? ( $conditions['groups'] ?? array() ) : array();
			if ( count( $groups ) > 1 ) {
				return $this->error( 'Free plan supports only 1 condition rule per field. Upgrade to Pro for condition groups.', 403 );
			}
		...
		$entity  = CA_Field::fromArray( array_merge( $this->transformField( $field ), $payload ) );
		$entity->id = $field->id;

		$id       = $this->repository->save( $entity );
		$updated  = $this->repository->findById( $id );
		if ( ! $updated instanceof CA_Field ) {
	...
	public function destroy( WP_REST_Request $request ) {
		...
		global $wpdb;
		$meta_key = '_ca_' . $field->field_key;
		$cache_key   = 'cecfe_postmeta_count_' . md5( $meta_key );
		$cache_group = 'coderembassy-checkout-field-editor';
	...
	public function revisions( WP_REST_Request $request ) {
		if ( ! \CoderEmbassy\CheckoutFieldEditor\Modules\Licensing\FeatureGate::can(
			\CoderEmbassy\CheckoutFieldEditor\Modules\Licensing\FeatureGate::REVISION_HISTORY
		) ) {
			return $this->error( 'Revision history is a Pro feature. Please activate your license.', 403 );
		}
	...
	private function transformField( CA_Field $field ): array {
		return array(
			'id'                  => $field->id,
	...
	private function sanitizeFieldPayload( array $data, ?CA_Field $existing = null ): array {
