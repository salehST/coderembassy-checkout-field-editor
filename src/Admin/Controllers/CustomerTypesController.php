...
class CustomerTypesController extends BaseController {
	...
	public function toggle_company( WP_REST_Request $request ) {
		...
		if ( $enabled ) {
			...
			if ( $existing instanceof CA_CustomerType ) {
				return $this->success(
					array(
						'enabled' => true,
			...
			$type = CA_CustomerType::fromArray(
				array(
					'slug'        => 'company',
					'label'       => \__( 'Company', 'coderembassy-checkout-field-editor' ),
					'description' => \__( 'Business or organization customer.', 'coderembassy-checkout-field-editor' ),
					'is_default'  => false,
					'priority'    => 20,
					'meta'        => array(),
				)
			...
			return $this->success(
				array(
					'enabled' => true,
					'type'    => $item instanceof CA_CustomerType ? $this->transform( $item ) : null,
				)
			);
		}

		$company = $this->repository->findBySlug( 'company' );
		if ( ! $company instanceof CA_CustomerType ) {
