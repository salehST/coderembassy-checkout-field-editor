...
use CoderEmbassy\CheckoutFieldEditor\Models\CA_Section;
...
class SectionsController extends BaseController {
	...
	public function create( WP_REST_Request $request ) {
		...
		// Free tier: max 1 section.
		if ( ! \CoderEmbassy\CheckoutFieldEditor\Modules\Licensing\FeatureGate::isPro() ) {
			$existing_count = count( $this->repository->findAll() );
			if ( $existing_count >= \CoderEmbassy\CheckoutFieldEditor\Modules\Licensing\FeatureGate::FREE_SECTIONS_MAX ) {
				return $this->error(
					sprintf(
						'Free plan allows up to %d section. Upgrade to Pro for unlimited sections.',
						\CoderEmbassy\CheckoutFieldEditor\Modules\Licensing\FeatureGate::FREE_SECTIONS_MAX
					),
					403
				);
			}
	...
	public function update( WP_REST_Request $request ) {
		...
		if ( ! $section instanceof CA_Section ) {
	...
	private function transform( CA_Section $section ): array {
		return array(
			'id'             => $section->id,
