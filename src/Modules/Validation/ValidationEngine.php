...
class ValidationEngine
{
	...
	public function validateCheckout(): void
	{
		try {
			...
			foreach ($fields as $field) {
				...
				// phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized -- dynamic key; sanitized immediately below via sanitize_text_field / array_map.
			...
			// phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedHooknameFound -- ca_ is the plugin's public hook prefix; cecfe_ is the legacy alias.
	...
	public function validateBlocksCheckout(\WC_Order $order, \WP_REST_Request $request): void
	{
		...
		// phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedHooknameFound -- ca_ is the plugin's public hook prefix.
		\do_action('ca_after_validation', $results, $context);
		\do_action('cecfe_after_validation', $results, $context);
	}
	...
	public function validateField(CA_Field $field, mixed $value, array $context): ValidationResult
	{
		...
		if (isset($rules['confirm']) && is_string($rules['confirm'])) {
			...
			if ('' === $confirm_value && isset($_POST[$confirm_key])) {
				$nonce_clean = isset($_POST['woocommerce-process-checkout-nonce'])
					? \sanitize_text_field(\wp_unslash((string) $_POST['woocommerce-process-checkout-nonce']))
					: (isset($_POST['nonce'])
