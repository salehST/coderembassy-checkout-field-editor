...
class ValidationEngine {
	...
	public function validateCheckout(): void {
		try {
			...
			// Validate classic checkout request nonce.
			...
			foreach ( $fields as $field ) {
				...
				// phpcs:ignore WordPress.Security.NonceVerification.Missing -- Nonce is verified in the next line.
				$nonce_val = isset( $_POST['woocommerce-process-checkout-nonce'] ) ? \sanitize_text_field( \wp_unslash( (string) $_POST['woocommerce-process-checkout-nonce'] ) ) : '';
				$nonce_ok  = ( '' !== $nonce_val && \wp_verify_nonce( $nonce_val, 'woocommerce-process_checkout' ) );

				if ( $nonce_ok && isset( $_POST[ $field->field_key ] ) ) {
					// phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized -- Sanitized in the next step based on type.
					$raw_post = \wp_unslash( $_POST[ $field->field_key ] );
			...
			if ( defined( 'WP_DEBUG' ) && WP_DEBUG ) {
				...
				error_log( '[Checkout Fields Manager] validateCheckout error: ' . $e->getMessage() . ' in ' . $e->getFile() . ':' . $e->getLine() );
			}
		}
	}
	...
	public function validateBlocksCheckout( \WC_Order $order, \WP_REST_Request $request ): void {
		...
		$fields = array_values(
			array_filter(
				...
				fn ( CECFM_Field $field ): bool => $this->isBlocksEligibleField( $field )
			)
		);
		...
		foreach ( $fields as $field ) {
			...
			$raw = $this->getBlocksRequestFieldValue(
				$request,
				$params,
				'coderembassy-checkout-fields-manager/' . sanitize_key( (string) $field->field_key ),
		...
		foreach ( $fields as $field ) {
			...
			if ( ! $result->passed ) {
				$order->add_meta_data( '_cecfm_validation_error', $result->error_message );
				throw new \Automattic\WooCommerce\StoreApi\Exceptions\RouteException(
					'cecfm_validation_error',
		...
		foreach ( $fields as $field ) {
			...
			$value = (string) ( $context['field_values'][ $field->field_key ] ?? '' );
			if ( '' !== $value ) {
				$order->update_meta_data( '_cecfm_' . $field->field_key, $value );
			}
		}
		if ( '' !== $context['customer_type'] ) {
	...
	public function validateField( CECFM_Field $field, mixed $value, array $context ): ValidationResult {
		...
		if ( $this->resolver->isFieldRequired( $field, $context ) && '' === trim( $string_value ) ) {
			...
			$result = ValidationResult::fail( $field->field_key, sprintf( __( '%s is required.', 'coderembassy-checkout-fields-manager' ), $field->label ) );
		...
		if ( ! empty( $rules['email'] ) && '' !== $string_value && false === filter_var( $string_value, FILTER_VALIDATE_EMAIL ) ) {
			/* translators: %s: the label shown for the checkout field. */
			$result = ValidationResult::fail( $field->field_key, sprintf( __( '%s must be a valid email address.', 'coderembassy-checkout-fields-manager' ), $field->label ) );
			return $this->filterFieldValidationResult( $result, $field, $value, $context );
		}
		...
		if ( '' !== $string_value ) {
			if ( isset( $rules['min_length'] ) && strlen( $string_value ) < (int) $rules['min_length'] ) {
				/* translators: %s: the label shown for the checkout field. */
				$result = ValidationResult::fail( $field->field_key, sprintf( __( '%s is too short.', 'coderembassy-checkout-fields-manager' ), $field->label ) );
				return $this->filterFieldValidationResult( $result, $field, $value, $context );
			}

			if ( isset( $rules['max_length'] ) && strlen( $string_value ) > (int) $rules['max_length'] ) {
				/* translators: %s: the label shown for the checkout field. */
				$result = ValidationResult::fail( $field->field_key, sprintf( __( '%s is too long.', 'coderembassy-checkout-fields-manager' ), $field->label ) );
				return $this->filterFieldValidationResult( $result, $field, $value, $context );
			}
			...
			if ( isset( $rules['regex'] ) ) {
				...
				if ( 1 !== $match ) {
					/* translators: %s: the label shown for the checkout field. */
					$result = ValidationResult::fail( $field->field_key, sprintf( __( '%s format is invalid.', 'coderembassy-checkout-fields-manager' ), $field->label ) );
					return $this->filterFieldValidationResult( $result, $field, $value, $context );
				}
			}
		}
		...
		if ( ! empty( $rules['numeric'] ) && '' !== $string_value && ! is_numeric( $string_value ) ) {
			/* translators: %s: the label shown for the checkout field. */
			$result = ValidationResult::fail( $field->field_key, sprintf( __( '%s must be numeric.', 'coderembassy-checkout-fields-manager' ), $field->label ) );
			return $this->filterFieldValidationResult( $result, $field, $value, $context );
		}
		...
		if ( '' !== $string_value && is_numeric( $string_value ) ) {
			if ( isset( $rules['min_value'] ) && is_numeric( (string) $rules['min_value'] ) && (float) $string_value < (float) $rules['min_value'] ) {
				/* translators: %s: the label shown for the checkout field. */
				$result = ValidationResult::fail( $field->field_key, sprintf( __( '%s is below the minimum allowed value.', 'coderembassy-checkout-fields-manager' ), $field->label ) );
				return $this->filterFieldValidationResult( $result, $field, $value, $context );
			}

			if ( isset( $rules['max_value'] ) && is_numeric( (string) $rules['max_value'] ) && (float) $string_value > (float) $rules['max_value'] ) {
				/* translators: %s: the label shown for the checkout field. */
				$result = ValidationResult::fail( $field->field_key, sprintf( __( '%s exceeds the maximum allowed value.', 'coderembassy-checkout-fields-manager' ), $field->label ) );
				return $this->filterFieldValidationResult( $result, $field, $value, $context );
			}
		}
		...
		if ( isset( $rules['confirm'] ) && is_string( $rules['confirm'] ) ) {
			...
			if ( '' === $confirm_value ) {
				// phpcs:ignore WordPress.Security.NonceVerification.Missing -- Nonce is verified in the next line.
				$nonce_val = isset( $_POST['woocommerce-process-checkout-nonce'] ) ? \sanitize_text_field( \wp_unslash( (string) $_POST['woocommerce-process-checkout-nonce'] ) ) : '';
				if ( '' !== $nonce_val && \wp_verify_nonce( $nonce_val, 'woocommerce-process_checkout' ) && isset( $_POST[ $confirm_key ] ) ) {
					$confirm_value = \sanitize_text_field( \wp_unslash( (string) $_POST[ $confirm_key ] ) );
				}
			}
			if ( $string_value !== $confirm_value ) {
				/* translators: %s: the label shown for the checkout field. */
				$result = ValidationResult::fail( $field->field_key, sprintf( __( '%s does not match confirmation.', 'coderembassy-checkout-fields-manager' ), $field->label ) );
				return $this->filterFieldValidationResult( $result, $field, $value, $context );
			}
		}
	 ...
	 * @return string[]
	 */
	private function getBlocksChoiceValues( CECFM_Field $field, \WC_Order $order, \WP_REST_Request $request, array $params, mixed $checkout_fields_service ): array {
		$selected = array();
		...
		foreach ( (array) $field->options as $option ) {
			...
			$raw        = $this->getBlocksRequestFieldValue(
				$request,
				$params,
				'coderembassy-checkout-fields-manager/' . $option_key,
				$option_key
			);
			if ( null === $raw ) {
				$raw = $this->getBlocksFieldValue( 'coderembassy-checkout-fields-manager/' . $option_key, $field->section, $order, $checkout_fields_service );
			}
	...
	private function getBlocksRequestFieldValue( \WP_REST_Request $request, array $params, string $namespaced_key, string $short_key ): mixed {
		...
		$nested_maps = array(
			...
			$params['extensions']['coderembassy-checkout-fields-manager'] ?? null,
			$params['extensions']['cecfm_pro'] ?? null,
		);
	...
	private function isBlocksEligibleField( CECFM_Field $field ): bool {
