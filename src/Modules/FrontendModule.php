...
use CoderEmbassy\CheckoutFieldEditor\Frontend\BlocksIntegration;
use CoderEmbassy\CheckoutFieldEditor\Frontend\CheckoutRenderer;
...
use CoderEmbassy\CheckoutFieldEditor\Modules\OrderMeta\OrderMetaHandler;
...
class FrontendModule extends AbstractModule {
	...
	public function boot(): void {
		...
		\add_action( 'wp_ajax_ca_set_customer_type', array( $renderer, 'ajaxSetCustomerType' ) );
		\add_action( 'wp_ajax_nopriv_ca_set_customer_type', array( $renderer, 'ajaxSetCustomerType' ) );
		...
		if ( ! empty( $settings['order_review_show_quantity'] ) ) {
			...
			\add_action( 'wp_ajax_ca_update_cart_qty', array( $renderer, 'ajaxUpdateCartQty' ) );
			\add_action( 'wp_ajax_nopriv_ca_update_cart_qty', array( $renderer, 'ajaxUpdateCartQty' ) );
		}
				...
				if ( ! empty( $types ) ) {
					...
					if ( ! empty( $type_options ) ) {
						\woocommerce_register_additional_checkout_field(
							array(
								'id'       => 'coderembassy-checkout-field-editor/ca_customer_type',
						...
						$registered['coderembassy-checkout-field-editor/ca_customer_type'] = true;
					}
				}
				foreach ( $field_repository->findAll( array( 'enabled' => true ) ) as $field ) {
					$field_key = \sanitize_key( (string) $field->field_key );
					if ( '' === $field_key || 'ca_customer_type' === $field_key ) {
						continue;
					}
					...
					$id = 'coderembassy-checkout-field-editor/' . $field_key;
					...
					if ( ! empty( $rules ) ) {
						...
						$args['validate_callback'] = static function ( $value ) use ( $rules, $label ) {
							...
							if ( ! empty( $rules['email'] ) && ! \is_email( $string ) ) {
								...
								return new \WP_Error( 'ca_invalid_email', sprintf( \__( '%s must be a valid email address.', 'coderembassy-checkout-field-editor' ), $label ) );
							}
							if ( isset( $rules['min_length'] ) && strlen( $string ) < (int) $rules['min_length'] ) {
								/* translators: %s: the label shown for the checkout field. */
								return new \WP_Error( 'ca_min_length', sprintf( \__( '%s is too short.', 'coderembassy-checkout-field-editor' ), $label ) );
							}
							if ( isset( $rules['max_length'] ) && strlen( $string ) > (int) $rules['max_length'] ) {
								/* translators: %s: the label shown for the checkout field. */
								return new \WP_Error( 'ca_max_length', sprintf( \__( '%s is too long.', 'coderembassy-checkout-field-editor' ), $label ) );
							}
							if ( isset( $rules['regex'] ) && '' !== (string) $rules['regex'] ) {
								...
								if ( 1 !== $match ) {
									/* translators: %s: the label shown for the checkout field. */
									return new \WP_Error( 'ca_invalid_pattern', sprintf( \__( '%s format is invalid.', 'coderembassy-checkout-field-editor' ), $label ) );
								}
							}
							if ( isset( $rules['min_value'] ) && is_numeric( $string ) && (float) $string < (float) $rules['min_value'] ) {
								/* translators: %s: the label shown for the checkout field. */
								return new \WP_Error( 'ca_min_value', sprintf( \__( '%s is below the minimum allowed value.', 'coderembassy-checkout-field-editor' ), $label ) );
							}
							if ( isset( $rules['max_value'] ) && is_numeric( $string ) && (float) $string > (float) $rules['max_value'] ) {
								/* translators: %s: the label shown for the checkout field. */
								return new \WP_Error( 'ca_max_value', sprintf( \__( '%s exceeds the maximum allowed value.', 'coderembassy-checkout-field-editor' ), $label ) );
							}
							return null;
						};
					}
		...
		\add_filter(
			...
			static function ( array $address, \WC_Order $order ) use ( $settings ): array {
				...
				foreach ( $keys as $key ) {
					$value = (string) $order->get_meta( '_ca_' . $key );
					if ( '' !== $value ) {
						$address[ $key ] = $value;
					}
				}
		...
		\add_action(
			...
			function (): void {
				...
				\wp_enqueue_style(
					'checkout-architect-frontend',
					CECFE_URL . 'assets/frontend/checkout.css',
					array(),
					CECFE_VERSION
				);
				\wp_enqueue_script(
					'checkout-architect-frontend',
					CECFE_URL . 'assets/frontend/checkout.js',
				...
				\wp_add_inline_style(
					'checkout-architect-frontend',
					'.ca-field--checkbox .ca-checkbox-label { display:inline-flex !important; align-items:center !important; gap:6px; cursor:pointer; font-weight:normal !important; line-height:1.4 !important; }
.ca-field--checkbox .ca-checkbox-label input[type="checkbox"] { margin:0; width:16px; height:16px; flex-shrink:0; }
.ca-field--checkbox .ca-field-description { display:block; margin-top:4px; font-size:0.875em; color:#666; }
.ca-field-inner { display:flex; flex-wrap:wrap; align-items:center; }
.ca-field-inner > label { width:100%; flex:0 0 100%; font-weight:normal; }
.ca-field-inner > .ca-currency { flex-shrink:0; font-weight:normal; line-height:normal; padding-right:2px; }
.ca-field-inner > input, .ca-field-inner > select, .ca-field-inner > textarea { flex:1; min-width:0; }
.ca-field-inner > .ca-field-description { width:100%; flex:0 0 100%; margin-top:4px; font-size:0.875em; color:#666; }
');

				// Inline CSS for order review enhancements.
				// Output merchant-authored custom CSS.
				if ( ! empty( $settings['custom_css'] ) ) {
					$safe_css = wp_strip_all_tags( (string) $settings['custom_css'] );
					\wp_add_inline_style( 'checkout-architect-frontend', $safe_css );
				}

				if ( ! empty( $settings['order_review_show_thumbs'] ) || ! empty( $settings['order_review_show_quantity'] ) ) {
					$inline_css = '
/* Product thumbnail in order review */
.ca-order-thumb-wrap { display:inline-block; vertical-align:middle; margin-right:8px; }
.ca-order-thumb-wrap img.ca-order-thumb { width:40px; height:40px; object-fit:cover; border-radius:4px; }
...
.ca-qty-btn:hover { background:#6d28d9; }
.ca-qty-remove { background:#e53e3e !important; font-size:11px !important; }
.ca-qty-remove:hover { background:#c53030 !important; }
.ca-qty-value { min-width:22px; text-align:center; font-weight:600; font-size:14px; flex-shrink:0; }
';
					\wp_add_inline_style( 'checkout-architect-frontend', $inline_css );
				}
...
(function(){
	document.addEventListener("click",function(e){
		var btn=e.target.closest(".ca-qty-btn");
		if(!btn)return;
		var wrap=btn.closest(".ca-qty-control");
		if(!wrap)return;
		var key=wrap.dataset.key;
		var nonce=wrap.dataset.nonce;
		var val=wrap.querySelector(".ca-qty-value");
		var isRemove=btn.classList.contains("ca-qty-remove");
		var qty;
		if(isRemove){
			qty=0;
		} else {
			var current=parseInt(val?val.textContent:0,10)||0;
			qty=btn.classList.contains("ca-qty-plus")?current+1:Math.max(1,current-1);
			if(val)val.textContent=qty;
		}
		// Disable buttons while request is in flight.
		wrap.querySelectorAll(".ca-qty-btn").forEach(function(b){b.disabled=true;});
		var params=new URLSearchParams({action:"ca_update_cart_qty",cart_key:key,qty:qty,nonce:nonce});
		fetch("' . \esc_js( $ajax_url ) . '",{method:"POST",headers:{"Content-Type":"application/x-www-form-urlencoded"},body:params.toString()})
			...
			.catch(function(){
				...
				wrap.querySelectorAll(".ca-qty-btn").forEach(function(b){b.disabled=false;});
			});
	});
})();
';
				...
				\wp_localize_script(
					...
					array(
						'ajax_url'     => \admin_url( 'admin-ajax.php' ),
						'type_nonce'   => \wp_create_nonce( 'ca_set_type' ),
				...
				if ( ! empty( $settings['enable_google_address_autofill'] ) && ! empty( $settings['google_maps_api_key'] ) ) {
					...
					\wp_enqueue_script(
						'checkout-architect-google-places',
...
';
					\wp_add_inline_script( 'checkout-architect-google-places', $inline_autofill_js );
				}
			},
			20
		);
