...
namespace CoderEmbassy\CheckoutFieldEditor\Modules\OrderMeta;
...
class OrderMetaHandler
{
	...
	public function saveCheckoutMeta(int $order_id, array $posted_data): void
	{
		try {
			...
			foreach ($section_counts as $section_key => $count) {
				$order->update_meta_data('_ca_repeat_' . $section_key, (int) $count);
			}
			...
			if (\defined('WP_DEBUG') && WP_DEBUG) {
				...
				\error_log('[Checkout Architect] saveCheckoutMeta error: ' . $e->getMessage());
			}
		}
	}
	...
	{
		if ('file' === $field->type) {
			// phpcs:ignore WordPress.Security.NonceVerification.Missing -- nonce verified in saveCheckoutMeta before this method is called.
			if (! isset($_FILES[$post_key]) || ! is_array($_FILES[$post_key])) {
				return;
			}
			// phpcs:ignore WordPress.Security.NonceVerification.Missing, WordPress.Security.ValidatedSanitizedInput.InputNotSanitized -- nonce verified upstream; wp_handle_upload performs its own validation.
			...
			$order->update_meta_data('_ca_' . $post_key, \esc_url_raw($uploaded['url']));
			return;
		}

		if ('repeater' === $field->type) {
			// phpcs:ignore WordPress.Security.NonceVerification.Missing, WordPress.Security.ValidatedSanitizedInput.InputNotSanitized -- nonce verified in saveCheckoutMeta; values are sanitized in the loop below.
			if (! isset($_POST[$post_key]) || ! is_array($_POST[$post_key])) {
				return;
			}
			// phpcs:ignore WordPress.Security.NonceVerification.Missing, WordPress.Security.ValidatedSanitizedInput.InputNotSanitized -- nonce verified upstream; values sanitized below.
			...
			$order->update_meta_data('_ca_' . $post_key, wp_json_encode($sanitized));
			return;
		}

		// phpcs:ignore WordPress.Security.NonceVerification.Missing, WordPress.Security.ValidatedSanitizedInput.InputNotSanitized -- nonce verified in saveCheckoutMeta; value is sanitized below.
		if (! isset($_POST[$post_key])) {
			return;
		}
		// phpcs:ignore WordPress.Security.NonceVerification.Missing, WordPress.Security.ValidatedSanitizedInput.InputNotSanitized -- nonce verified upstream; sanitized immediately below.
		...
		$order->update_meta_data('_ca_' . $post_key, $value);
	}

	/**
	 * @param \CoderEmbassy\CheckoutFieldEditor\Models\CA_Field[] $fields
	...
	private function buildDisplaySuffixMap(array $fields, \WC_Order $order): array
	{
		...
		foreach ($sections as $section) {
			...
			$count = (int) $order->get_meta('_ca_repeat_' . $section->section_key);
			if ($count < 1) {
				continue;
			}
	...
	public function displayOrderMeta(int|\WC_Order $order_or_id): void
	{
		try {
			...
			echo '<section class="ca-order-meta woocommerce-order-details">';
			echo '<h2 class="woocommerce-column__title">' . \esc_html__('Additional Information', 'coderembassy-checkout-field-editor') . '</h2>';
			echo '<table class="woocommerce-table shop_table ca-order-meta-table"><tbody>';
			...
			if (\defined('WP_DEBUG') && WP_DEBUG) {
				// phpcs:ignore WordPress.PHP.DevelopmentFunctions.error_log_error_log
				\error_log('[Checkout Architect] displayOrderMeta error: ' . $e->getMessage());
			}
		}
	}
	...
	public function displayAdminOrderMeta(\WC_Order $order): void
	{
		try {
			...
			foreach ($fields as $field) {
				...
				if (isset($suffix_map[$field->field_key])) {
					foreach ($suffix_map[$field->field_key] as $sfx) {
						$raw_value = $order->get_meta('_ca_' . $field->field_key . $sfx);
				...
				$raw_value = $order->get_meta('_ca_' . $field->field_key);
				if (null === $raw_value || false === $raw_value || '' === (string) $raw_value) {
					continue;
				}
				$admin_value = 'date' === $field->type
			...
			echo '<div class="ca-order-meta-admin" style="margin-top:12px">';
			echo '<h4 style="margin:0 0 4px">' . \esc_html__('Custom Checkout Fields', 'coderembassy-checkout-field-editor') . '</h4>';
			// phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
			echo $items;
			echo '</div>';
		} catch (\Throwable $e) {
			if (\defined('WP_DEBUG') && WP_DEBUG) {
				// phpcs:ignore WordPress.PHP.DevelopmentFunctions.error_log_error_log
				\error_log('[Checkout Architect] displayAdminOrderMeta error: ' . $e->getMessage());
			}
		}
	}
	...
	public function displayEmailMeta(\WC_Order $order, bool $sent_to_admin, bool $plain_text): void
	{
		try {
			$fields     = $this->fieldRepository->findAll(array('enabled' => true));
			$settings   = \get_option('checkout_architect_settings', '');
			$settings   = is_string($settings) && '' !== $settings ? (\json_decode($settings, true) ?? array()) : array();
			$heading    = ! empty($settings['email_section_heading']) ? $settings['email_section_heading'] : \__('Additional Information', 'coderembassy-checkout-field-editor');
			...
			foreach ($fields as $field) {
				...
				if (isset($suffix_map[$field->field_key])) {
					foreach ($suffix_map[$field->field_key] as $sfx) {
						$sfx_value = $order->get_meta('_ca_' . $field->field_key . $sfx);
				...
				if ('checkbox' === $field->type) {
					$display = ('1' === (string) $raw_value) ? \__('Yes', 'coderembassy-checkout-field-editor') : '';
					if ('' === $display) {
						continue;
					}
					...
					foreach ($decoded as $i => $row) {
						...
						$lines[] = \sprintf(
							...
							\__('Row %d', 'coderembassy-checkout-field-editor'),
							(int) $i + 1
						) . ': ' . implode(' | ', $pair_strs);
					}
					if (empty($lines)) {
			...
			if ($plain_text) {
				...
				foreach ($rows as $row) {
					echo '<tr>';
					// phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
					echo '<th scope="row" style="text-align:left;border:1px solid #eee;padding:12px;color:#636363;">' . $row['label'] . '</th>';
					// phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
					echo '<td style="text-align:left;border:1px solid #eee;padding:12px;color:#636363;">' . $row['display'] . '</td>';
					echo '</tr>';
				}
				echo '</tbody></table>';
			}
			...
			if (\defined('WP_DEBUG') && WP_DEBUG) {
				// phpcs:ignore WordPress.PHP.DevelopmentFunctions.error_log_error_log
				\error_log('[Checkout Architect] displayEmailMeta error: ' . $e->getMessage());
			}
		}
	}
	 ...
	 *
	 * @param array<string, mixed> $settings Decoded checkout_architect_settings.
	 */
