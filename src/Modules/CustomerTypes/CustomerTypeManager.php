...
class CustomerTypeManager {
	...
	public function getCurrentType(): ?CA_CustomerType {
		$session_slug = '';
		if ( function_exists( 'WC' ) && \WC()->session ) {
			$session_slug = (string) \WC()->session->get( 'checkout_architect_customer_type', '' );
		}

		$user_slug = '';
		if ( \is_user_logged_in() ) {
			$user_slug = (string) \get_user_meta( \get_current_user_id(), 'checkout_architect_customer_type', true );
		}

		$current_slug = $session_slug ?: $user_slug;
		if ( '' === $current_slug ) {
			$default = $this->repository->getDefault();
			if ( $default instanceof CA_CustomerType ) {
		...
		$filtered_slug = (string) \apply_filters( 'ca_current_customer_type', $current_slug, $context );
		return $this->repository->findBySlug( $filtered_slug );
	}

	/**
	 * @return CA_CustomerType[]
	 */
	...
	public function setCurrentType( string $slug ): void {
		...
		if ( function_exists( 'WC' ) && \WC()->session ) {
			\WC()->session->set( 'checkout_architect_customer_type', $slug );
		}

		if ( \is_user_logged_in() ) {
			\update_user_meta( \get_current_user_id(), 'checkout_architect_customer_type', $slug );
		}

		// phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedHooknameFound
		\do_action( 'ca_customer_type_changed', $slug, $old_slug );
	}
	...
	public function fieldBelongsToType( CA_Field $field, string $type_slug ): bool {
	...
	public function getTypeSelectorConfig(): array {
		...
		return array(
			...
			'type'      => 'select',
			'label'     => \__( 'Customer Type', 'coderembassy-checkout-field-editor' ),
			'options'   => $items,
	...
	public function getLabelForType( CA_Field $field, string $type_slug ): string {
