...
class CustomerTypeRepository extends AbstractRepository {
	public function __construct( \wpdb $db ) {
		...
		$this->table_name = $this->db->prefix . 'ca_customer_types';
	}
	...
	public function findById( int $id ): ?CA_CustomerType {
		$model = parent::findById( $id );
		return $model instanceof CA_CustomerType ? $model : null;
	}

	public function findBySlug( string $slug ): ?CA_CustomerType {
	...
	public function save( mixed $entity ): int {
		if ( ! $entity instanceof CA_CustomerType ) {
			throw new InvalidArgumentException( 'CustomerTypeRepository expects a CA_CustomerType entity.' );
		}
	...
	public function getDefault(): ?CA_CustomerType {
	...
	protected function toModel( array $row ): CA_CustomerType {
		if ( isset( $row['meta'] ) && is_string( $row['meta'] ) ) {
			$row['meta'] = $this->jsonDecode( $row['meta'] );
		}

		return CA_CustomerType::fromRow( $row );
	}
}
