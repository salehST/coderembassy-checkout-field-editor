/* global CECFM_Frontend, jQuery */
( function ( $ ) {
	'use strict';

	var CA = window.CECFM_Frontend || {};
	 ...
	 *
	 * Custom fields: rendered with data-cecfm-types="slug1,slug2" on the
	 *   <p id="fieldkey_field"> wrapper. Empty = show for all types.
	 *
	 * Native WC fields: restriction map passed via CECFM_Frontend.native_types
	...
	function applyTypeVisibility( slug ) {
		...
		// 1. Custom CA fields with data-cecfm-types attribute.
		$( '[data-cecfm-types]' ).each( function () {
			var raw   = String( $( this ).attr( 'data-cecfm-types' ) || '' );
	 ...
	 * ------------------------------------------------------------- */
	$( document ).on( 'click', '.cecfm-type-btn', function () {
		var $btn = $( this );
		...
		currentType = slug;
		$( '.cecfm-type-btn' ).removeClass( 'is-active active' );
		...
		$( '#cecfm-current-type-input' ).val( slug );
		...
		$.post( CA.ajax_url, {
			action:         'cecfm_set_customer_type',
			type:           slug,
			_cecfm_type_nonce: CA.type_nonce,
		} );
	} );
	 ...
	 * ------------------------------------------------------------- */
	$( function () {
		var $active = $( '.cecfm-type-btn.is-active' );
		if ( $active.length ) {
	...
	function applyHiddenFields() {
		$( '.cecfm-field-hidden' ).closest( '.form-row' ).hide();
	}

	applyHiddenFields();

	/* ---------------------------------------------------------------
	 * File Upload — add multipart/form-data to checkout form.
	 * ------------------------------------------------------------- */
	if ( $( 'input[type="file"]' ).length ) {
		$( 'form.woocommerce-checkout' ).attr( 'enctype', 'multipart/form-data' );
	}

	/* ---------------------------------------------------------------
	 * Repeater Field — Add/Remove rows.
	 * ------------------------------------------------------------- */
	$( document ).on( 'click', '.cecfm-repeater-add', function ( e ) {
		e.preventDefault();
		var $wrap = $( this ).closest( '.cecfm-repeater-wrap' );
		var $rows = $wrap.find( '.cecfm-repeater-rows' );
		var max   = parseInt( $wrap.data( 'max-rows' ), 10 ) || 999;

		if ( $rows.children( '.cecfm-repeater-row' ).length >= max ) {
			return;
		}

		var tmpl = $wrap.data( 'template' );
		var idx  = $wrap.data( 'next-idx' ) || 1;
		if ( ! tmpl ) {
			return;
		}

		var html = tmpl.replace( /__IDX__/g, String( idx ) );
		$rows.append( html );
		$wrap.data( 'next-idx', idx + 1 );
	} );

	$( document ).on( 'click', '.cecfm-repeater-remove', function ( e ) {
		e.preventDefault();
		$( this ).closest( '.cecfm-repeater-row' ).remove();
	} );
